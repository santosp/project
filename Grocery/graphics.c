...
bounding_box_t draw_bmp(unsigned char x, unsigned char y,const unsigned char *bmp) {
	unsigned short pos=BMP_POS_START;
	bounding_box_t ret;
	unsigned char i;//loop counter for width of bmp
	unsigned char j;//loop counter for 1 collum byte of bmp
	unsigned char page;//loop counter for page count
	unsigned char bmp_pages_used;//the number of pages used in the bmp
	ret.x1 = x;
	ret.y1 = y;
	ret.x2 = x;
	ret.y2 = y;
	...
	bmp_pages_used=bmp[BMP_HEIGHT]/8;

	// Draw page 0 to 7
	for(page=0;page<=bmp_pages_used;page++){
		// Draw left to right
		for (i = 0; i < bmp[BMP_WIDTH]; i++) {
			...
			for ( j = 0; j < 8; j++) {
				...
				//p=8*(k);
				if (bmp[pos] & 1 << (j % 8)) {
					glcd_pixel(x + i, y + j + (8*page), 1);
				} else {
					glcd_pixel(x + i, y + j + (8*page), 0);
				}
		...
		ret.x2 = ret.x1 + bmp[BMP_WIDTH] - 1;
		// TODO: Return the actual height drawn, rather than the height of the
		//		 font.
		//ret.y2 = ret.y1 + height;
		ret.y2 = ret.y1 +bmp[BMP_HEIGHT];
	}
